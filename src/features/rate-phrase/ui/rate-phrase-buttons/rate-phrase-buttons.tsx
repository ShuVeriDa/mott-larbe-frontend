...
import type { PhraseReviewQuality } from "@/entities/phrasebook";

type RatingKey = "again" | "hard" | "good" | "easy";

interface RatingOption {
	key: RatingKey;
	quality: PhraseReviewQuality;
	icon: string;
	hoverClass: string;
}
...
export interface RatePhraseButtonsProps {
	visible: boolean;
	disabled?: boolean;
	onRate: (quality: PhraseReviewQuality) => void;
}

export const RatePhraseButtons = ({ visible, disabled, onRate }: RatePhraseButtonsProps) => {
	const { t } = useI18n();
		...
		>
				...
				{RATINGS.map((rating) => {
					...
					return (
						<Button
							...
							className={cn(
								"flex flex-col items-center gap-1 rounded-card border-[0.5px] border-bd-2 bg-surf py-2.5 px-1.5",
						...
						>
							<Typography tag="span" className="text-[17px] leading-none">
								{rating.icon}
							</Typography>
			...
			<div className="mt-2 flex items-center justify-center gap-3.5 max-md:hidden">
				{RATINGS.map((rating, idx) => (
					<div key={rating.key} className="flex items-center gap-1 text-[11px] text-t-3">
