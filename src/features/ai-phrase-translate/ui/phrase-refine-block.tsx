...
export const PhraseRefineBlock = ({ refineState, onOpen, onSubmit }: PhraseRefineBlockProps) => {
  const { t } = useI18n();
  ...
  if (refineState.phase === "error") {
    return (
      <div className="border-t border-hairline border-bd-1 px-3.5 py-2.5">
        <p className="text-[12px] text-red-t">{t("aiTranslation.phrase.refineError")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="border-t border-hairline border-bd-1 bg-pur-bg/40 px-3.5 py-2.5">
      ...
      <div className="text-[13.5px] font-medium text-t-1">{refineState.result.translation}</div>
      {refineState.result.notes && (
        <div className="mt-1 text-[11.5px] text-t-3">{refineState.result.notes}</div>
      )}
    </div>
  );
};
