...
export const LogsTable = ({ items, isLoading, onRowClick }: LogsTableProps) => {
	...
	if (isLoading) {
		return (
			<div className="hidden overflow-x-auto [&::-webkit-scrollbar]:h-0 md:block">
				<Table className="border-collapse text-[12.5px]" aria-busy="true" aria-label="Loading logs">
					<TableHeader>
						<LogsTableHead t={t} />
					</TableHeader>
					<TableBody>
	...
	return (
		<div className="hidden overflow-x-auto [&::-webkit-scrollbar]:h-0 md:block">
			<Table className="border-collapse text-[12.5px]" aria-label={t("admin.logs.table.message")}>
				<TableHeader>
					<LogsTableHead t={t} />
				</TableHeader>
				<TableBody>
...
return (
							...
							>
								...
								<TableCell className="px-3 py-2 max-[760px]:hidden">
									<DurationBadge durationMs={item.durationMs} />
								</TableCell>
								<TableCell className="px-3 py-2 max-[900px]:hidden">
...
const LogsTableHead = ({ t }: { t: (key: string) => string }) => (
	<TableRow className="border-b border-bd-1">
		{[
			...
			<TableHead
				key={key}
				className={`bg-surf-2 px-3 py-[9px] text-[10.5px] font-semibold uppercase tracking-[0.4px] text-t-3 whitespace-nowrap ${style}`}
			>
