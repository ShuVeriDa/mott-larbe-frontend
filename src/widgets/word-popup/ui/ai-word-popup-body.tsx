...
export const AiWordPopupBody = ({ word, normalized, contextSentence, lang }: AiWordPopupBodyProps) => {
  const { t } = useI18n();
    ...
    <>
      <div className="border-b border-hairline border-bd-1 px-3.5 pt-3.5 pb-2.5">
        <div className="mb-1.5 flex items-start gap-2">
          <div className="text-[17px] font-semibold tracking-[-0.2px] text-t-1">{word}</div>
      ...
      <div className="border-b border-hairline border-bd-1 px-3.5 py-2.5">
        <div className="flex items-start justify-between gap-2">
          <div className="text-[14px] font-medium text-t-1">{result.translation}</div>
          <Button
            size="bare"
            onClick={handleCopy}
        ...
        <div className={cn(
          ...
          {result.fromCache && result.status === "APPROVED"
            ? <><CheckCircle2 className="size-2.5" strokeWidth={1.8} />{t("aiTranslation.popup.sourceApproved")}</>
            : result.fromCache
              ? <><Clock className="size-2.5" strokeWidth={1.8} />{t("aiTranslation.popup.sourcePending")}</>
              : <><Sparkles className="size-2.5" strokeWidth={1.8} />{t("aiTranslation.popup.sourceFresh")}</>}
        </div>
        {result.partOfSpeech && (
        ...
        {result.example && (
          <div className="mt-1.5 text-[11.5px] text-t-3 italic">{result.example}</div>
        )}
      </div>
