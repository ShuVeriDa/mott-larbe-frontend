...
export const ExportTextsModal = ({
	...
	const { data, isLoading } = useAdminTexts({ limit: 100, sortBy: "createdAt", sortOrder: "desc" });
	const allTexts: AdminTextListItem[] = data?.items ?? [];

	useEffect(() => {
		if (!isLoading && allTexts.length > 0 && checkedIds.size === 0 && preselectedIds.length === 0) {
			setCheckedIds(new Set(allTexts.map((t) => t.id)));
		}
	}, [isLoading, allTexts.length]);
	...
	const handleExport = async () => {
		...
		try {
			...
			await adminTextApi.exportTexts({}, format, ids.length > 0 ? ids : undefined);
			onClose();
		} catch {
						...
						<div className="flex items-center justify-between">
							...
							{!isLoading && allTexts.length > 0 && (
								<Typography tag="span" className="text-[11px] text-t-4">
									{t("admin.texts.export.checkedCount", { count: checkedCount, total: allTexts.length })}
								</Typography>
							)}
						</div>
						...
						<div className="overflow-hidden rounded-[8px] border border-bd-1 bg-surf-2">
							...
							{!isLoading && filtered.length > 0 && (
								...
								>
									...
									<Typography tag="span" className="text-[12px] font-medium text-t-2">
										{allFilteredChecked
							...
							<div className="max-h-[220px] overflow-y-auto [&::-webkit-scrollbar]:w-0">
								{isLoading ? (
									...
									<Typography tag="p" className="px-3 py-4 text-center text-[12px] text-t-4">
...
const TextCheckRow = ({ text, checked, onToggle }: TextCheckRowProps) => {
	...
	return (
		...
		>
			...
			<Typography tag="span" className="min-w-0 flex-1 truncate text-[12.5px] text-t-1">
				{text.title}
			</Typography>
			{text.level && (
