		...
		>
			<div
				className={cn(
					...
					"max-sm:w-full max-sm:max-h-[92dvh] max-sm:overflow-y-auto max-sm:rounded-t-[20px] max-sm:rounded-b-none max-sm:pb-[calc(20px+env(safe-area-inset-bottom,0px))]",
					isOpen
			...
			>
				...
				<div className="mb-[18px] flex items-center justify-between">
					...
					<button
						type="button"
						onClick={onClose}
						className="flex size-7 items-center justify-center rounded-[7px] border border-bd-1 bg-surf-2 text-t-2 transition-colors hover:bg-surf-3"
					>
						<svg
							viewBox="0 0 12 12"
							fill="none"
					...
					<div className="grid grid-cols-2 gap-[7px]">
						{TYPE_OPTIONS.map((opt) => (
							...
							>
								<div
									className={cn(
										"flex size-7 shrink-0 items-center justify-center rounded-[7px] text-sm",
										opt.bgClass,
									)}
								>
									{opt.emoji}
				...
				<div className="mt-4 flex gap-2">
					<button
						type="button"
						onClick={onClose}
						className="flex-1 h-9 rounded-[9px] border border-bd-2 bg-surf-2 font-[inherit] text-[13px] font-medium text-t-2 transition-colors hover:bg-surf-3"
					>
