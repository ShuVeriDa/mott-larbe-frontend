...
export const DashboardAiCacheCard = ({ aiCache }: DashboardAiCacheCardProps) => {
  const { t } = useI18n();
  ...
  return (
    <div className="overflow-hidden rounded-[12px] border border-bd-1 bg-surf transition-colors">
      ...
      <div className="grid grid-cols-3 gap-px border-t border-hairline border-bd-1 bg-bd-1">
        <div className="bg-surf px-4 py-3">
          <div className="mb-0.5 text-[11px] text-t-3">{t("aiTranslation.admin.widget.pending")}</div>
          <div
            className={
              aiCache.pending > 0
        ...
        <div className="bg-surf px-4 py-3">
          <div className="mb-0.5 text-[11px] text-t-3">{t("aiTranslation.admin.widget.approvedWeek")}</div>
          <div className="text-[20px] font-bold text-grn">
            {aiCache.approvedThisWeek.toLocaleString("ru-RU")}
          </div>
        </div>
        <div className="bg-surf px-4 py-3">
          <div className="mb-0.5 text-[11px] text-t-3">{t("aiTranslation.admin.widget.notExported")}</div>
          <div
            className={
      ...
      {aiCache.topWords.length > 0 && (
        <div className="border-t border-hairline border-bd-1 px-4 py-3">
          ...
          <div className="flex flex-col gap-1.5">
            {aiCache.topWords.slice(0, 5).map((w) => (
              <div key={w.lemma} className="flex items-center justify-between gap-2">
                <div className="min-w-0">
                  <span className="text-[12.5px] font-medium text-t-1">{w.lemma}</span>
                  {w.translation && (
                    <span className="ml-1.5 text-[11.5px] text-t-3">{w.translation}</span>
                  )}
                </div>
                <span className="shrink-0 text-[11px] text-t-3">×{w.requestCount}</span>
              </div>
            ))}
          </div>
        </div>
