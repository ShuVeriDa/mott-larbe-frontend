...
export const TopWordsCard = ({ data }: TopWordsCardProps) => {
	...
	const isEmpty = isHardest ? data.hardest.length === 0 : data.recentlyMastered.length === 0;
				...
				<div className="flex items-center gap-0.5 rounded-base bg-surf-2 p-0.5">
					<button
						...
						className={`rounded-[5px] px-2.5 py-1 text-[11px] font-medium transition-colors ${
							isHardest ? "bg-surf text-t-1 shadow-sm" : "text-t-3 hover:text-t-2"
						}`}
					>
				...
				<div className="flex flex-col gap-1.5">
					{data.hardest.map((w, i) => (
						<div key={i} className="flex items-center gap-2">
							<Typography tag="span" className="w-4 shrink-0 text-[10px] text-t-3">{i + 1}.</Typography>
							<div className="min-w-0 flex-1">
								<Typography tag="span" className="block truncate text-[12px] font-semibold text-t-1">
									{w.word}
								</Typography>
								<Typography tag="span" className="block truncate text-[10.5px] text-t-3">
									{w.translation}
								</Typography>
							</div>
