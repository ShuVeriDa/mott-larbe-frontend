...
import type { Editor, SlashMenuItem, TipTapDoc } from "@/shared/ui/notion-editor";
	...
	return (
		<div ref={ref} className="relative">
			<button
				...
				className="flex h-7 items-center gap-1 rounded-[5px] px-1.5 text-[11px] font-semibold text-white/80 transition-colors hover:bg-white/15 hover:text-white"
			>
				Ӏ
			</button>
			{open && (
...
export const TextEditEditor = ({
	...
	const extraToolbarItems = useMemo(
		() => (
			<CharsPopup onInsert={char => editorRef.current?.chain().focus().insertContent(char).run()} />
		),
		[],
	);
	...
	return (
		<div className="flex min-w-0 flex-col border-r border-bd-1 max-[900px]:border-r-0">
			...
			{/* ── Pages bar ── */}
			<div className="sticky top-[52px] z-10 flex items-center overflow-x-auto border-b border-bd-1 bg-surf-2 px-3.5 transition-colors [scrollbar-width:none]">
				{pages.map((_, i) => (
					<button
						key={i}
						type="button"
			...
			<div className="group relative flex-1 px-[22px] py-[22px] pb-10 max-sm:px-4">
				<NotionEditor
					...
					onEditorReady={(ed) => { editorRef.current = ed; setEditor(ed); }}
				/>
			</div>
