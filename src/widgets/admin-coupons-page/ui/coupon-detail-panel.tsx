...
export const CouponDetailPanel = ({
	...
	const handleEdit: NonNullable<ComponentProps<"button">["onClick"]> = () => onEdit(coupon.id);
	const handleDelete: NonNullable<ComponentProps<"button">["onClick"]> = () => onDelete(coupon.id);

	return (
		<div className="flex flex-col gap-0 overflow-hidden rounded-card border border-bd-1 bg-surf">
			<CouponHeroSection coupon={coupon} copied={copied} onCopy={handleCopy} t={t} />
			<UsageSection coupon={coupon} t={t} />
			<RedemptionsList coupon={coupon} t={t} />
