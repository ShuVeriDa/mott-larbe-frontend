...
const IconSortable = ({
	...
	if (active && dir === "asc")
		return <ArrowUp className={cn("ml-1 size-[10px] text-acc")} />;
	if (active && dir === "desc")
		return <ArrowDown className={cn("ml-1 size-[10px] text-acc")} />;
...
export const DictionaryTable = ({
	...
	const getSortableHeaderProps = (
		...
		const dir: "asc" | "desc" | undefined =
			sort === sortAsc ? "asc" : sort === sortDesc ? "desc" : undefined;
		const handleClick = () => {
		...
		return { isActive, dir, onClick: handleClick };
	};
									...
									>
										...
										<td className="py-3 pl-3.5">
											...
											{item.translation && (
												<Typography
													tag="p"
													className="mt-0.5 text-[11.5px] text-t-3 line-clamp-1"
												>
													{item.translation}
												</Typography>
											)}
										...
										<td className="py-3 pl-3.5">
											<PosBadge
												...
												label={
													item.partOfSpeech
														? t(
																`admin.dictionary.pos.${item.partOfSpeech.toLowerCase()}`,
															)
														: undefined
												}
											/>
										...
										<td className="py-3 pl-3.5">
											{item.sensesCount > 0 ? (
												...
												<Typography
													tag="span"
													className="text-red-t text-[11.5px]"
												>
													0
												</Typography>
											)}
										...
										<td className="py-3 pl-3.5">
											{item.level ? (
												<CefrBadge
													level={
														item.level as import("@/shared/types").CefrLevel
													}
												/>
											) : (
												...
												<div className="flex flex-col gap-0.5">
													...
													<div className="h-[3px] w-full rounded-full bg-surf-3">
														<div
															...
															style={{
																width: `${Math.round((item.frequency / maxFrequency) * 100)}%`,
															}}
														/>
													</div>
												</div>
