...
export const GenreCard = ({ genre, lang, style, index }: GenreCardProps) => {
	...
	const hoverProps = useHoverProps(
		{ y: -4 },
		{ scale: 0.97 },
		{ duration: 0.35, ease: [0.22, 1, 0.36, 1], delay: index * 0.04 },
	);
	...
	return (
		...
		>
			<Link
				href={`/${lang}/texts?genre=${genre.id}`}
			...
			>
				<div
					className={`absolute inset-0 bg-linear-to-br ${style.gradient} transition-opacity duration-200`}
				/>
