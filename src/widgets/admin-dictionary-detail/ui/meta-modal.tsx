...
import type { AdminDictEntryCard, PatchAdminEntryDto } from "@/entities/dictionary";
...
interface MetaModalProps {
	...
	isPending: boolean;
	onClose: () => void;
	onSave: (body: PatchAdminEntryDto) => void;
}

export const MetaModal = ({ isOpen, data, isPending, onClose, onSave }: MetaModalProps) => {
	const { t } = useI18n();
	...
	useEffect(() => {
		if (isOpen && data) {
			...
			setLevel(data.level ?? "");
			setFrequency(data.frequency != null ? String(data.frequency) : "");
			setNotes(data.notes ?? "");
