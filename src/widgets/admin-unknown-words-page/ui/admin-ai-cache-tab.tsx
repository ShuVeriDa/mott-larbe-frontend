...
import type { AiCacheStatus, AiCacheEntry } from "@/entities/ai-translation";
import { cn } from "@/shared/lib/cn";
import { useI18n } from "@/shared/lib/i18n";
import { Button } from "@/shared/ui/button";
import { Typography } from "@/shared/ui/typography";
import { Check, Search, ThumbsDown, ThumbsUp, Trash2, Upload, X } from "lucide-react";
  ...
  return (
    <div className="mb-2.5 overflow-x-auto [&::-webkit-scrollbar]:h-0">
      <div className="flex w-fit gap-0.5 rounded-[9px] border border-bd-1 bg-surf-2 p-[3px]">
        {STATUS_TABS.map((s) => {
          const handleClick: NonNullable<ComponentProps<"button">["onClick"]> = () => onChange(s);
          return (
            <Button
              key={s}
              onClick={handleClick}
              ...
              {s === "PENDING" && pendingCount !== undefined && (
                <Typography
                  ...
                  className={cn(
                    ...
                    pendingCount > 0 ? "bg-red-bg text-red-t" : "bg-surf-3 text-t-3",
                  )}
                >
      ...
      <div className="flex items-start gap-3">
        <div className="min-w-0 flex-1">
          <div className="mb-1 flex flex-wrap items-center gap-2">
            <span className="font-semibold text-[14px] text-t-1">{entry.lemma}</span>
            <span
              className={cn(
                "shrink-0 rounded-[4px] px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-[0.4px] border-hairline",
            ...
            {entry.status === "APPROVED" && entry.exportedAt && (
              <span className="shrink-0 rounded-[4px] border-hairline border-grn/30 bg-grn/10 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-[0.4px] text-grn">
                {t("aiTranslation.admin.exported")}
              </span>
            )}
            {entry.requestCount >= 10 && (
              <span className="shrink-0 rounded-[4px] border-hairline border-red/30 bg-red-bg px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-[0.4px] text-red-t">
          ...
          <div className="mt-1.5 flex items-center gap-3 text-[11px] text-t-3">
            <span>{t("aiTranslation.admin.requestCount")}: <strong className="text-t-2">{entry.requestCount}</strong></span>
        ...
        <div className="flex shrink-0 items-center gap-1">
          {entry.status !== "APPROVED" && (
            <Button
              ...
              className="inline-flex h-7 w-7 items-center justify-center rounded-base border-hairline border-bd-1 bg-surf-2 text-grn transition-colors hover:bg-grn/10 disabled:opacity-50"
            >
          ...
          {entry.status !== "REJECTED" && (
            <Button
              ...
              className="inline-flex h-7 w-7 items-center justify-center rounded-base border-hairline border-bd-1 bg-surf-2 text-t-3 transition-colors hover:bg-red/10 hover:text-red disabled:opacity-50"
            >
  ...
  return (
    <div className="flex items-center justify-center gap-2 px-4 py-3 text-[12.5px] text-t-2">
      <Button
        ...
        onClick={handlePrev}
        disabled={page <= 1}
        className="rounded-base border-hairline border-bd-1 bg-surf-2 px-3 py-1 text-[12.5px] transition-colors hover:bg-surf-3 disabled:opacity-40"
      >
        ←
      </Button>
...
export const AdminAiCacheTab = () => {
  ...
  const handleSearchChange: NonNullable<ComponentProps<"input">["onChange"]> = (e) =>
    handleQChange(e.currentTarget.value);
    ...
    <div>
      <div className="mb-2.5 flex items-center justify-between gap-2">
        ...
        <Button
          ...
          className="flex shrink-0 items-center gap-1.5 rounded-base border-hairline border-bd-1 bg-surf-2 px-3 py-1.5 text-[12.5px] text-t-2 transition-colors hover:bg-surf-3 hover:text-t-1 disabled:opacity-50"
        >
