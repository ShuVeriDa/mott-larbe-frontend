		...
		<SettingCard title={t("profile.subscription.paymentHistory")} noBody>
			{payments.length === 0 ? (
				<div className="flex flex-col items-center gap-2 px-4 py-8 text-center">
					<Typography
						tag="span"
						className="flex size-9 items-center justify-center rounded-[9px] bg-surf-2"
					>
						<CardIcon />
					</Typography>
				...
				payments.map(p => (
					...
					>
						...
						<Typography
							tag="span"
							className="text-[12.5px] font-semibold text-t-1 shrink-0"
						>
							{formatPrice(p.amountCents, p.currency)}
						</Typography>
						<Typography
							tag="span"
							className={`px-1.5 py-0.5 rounded text-[10.5px] font-medium shrink-0 ${STATUS_STYLE[p.status]}`}
						>
