  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      {entries.map(entry => (
        <div
          key={entry.word}
          className="rounded-[8px] border border-hairline border-bd-1 bg-surf-2 px-3 py-2.5"
        >
          <div className="mb-1 flex items-start justify-between gap-2">
            <div className="text-[14px] font-semibold text-t-1">{entry.word}</div>
            <span
              className={cn(
                "mt-0.5 shrink-0 rounded-[4px] px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-[0.4px]",
                entry.translation.status === "APPROVED"
                  ? "bg-grn/15 text-grn border-hairline border-grn/30"
                  : "bg-pur/10 text-pur-t border-hairline border-pur/30",
              )}
            >
              {entry.translation.status === "APPROVED"
          ...
          <div className="text-[13px] text-t-2">{entry.translation.translation}</div>
          {entry.translation.partOfSpeech && (
            <div className="mt-0.5 text-[11px] text-t-3">
              {t("aiTranslation.popup.partOfSpeech")}: {entry.translation.partOfSpeech}
            </div>
          )}
          {entry.refinedTranslation && (
            <div className="mt-1.5 rounded-[6px] border border-hairline border-pur/20 bg-pur-bg/50 px-2 py-1.5">
              ...
              <div className="text-[12px] font-medium text-t-1">{entry.refinedTranslation}</div>
            </div>
          )}
        </div>
      ))}
...
export const ReaderAiHistoryAside = ({ open, onClose }: ReaderAiHistoryPanelProps) => {
  useEffect(() => {
    if (!open) return;
...
export const ReaderAiHistorySheet = ({ open, onClose }: ReaderAiHistoryPanelProps) => {
  const { t } = useI18n();
