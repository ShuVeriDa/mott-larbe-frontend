...
export type AiRefineState =
	| { phase: "idle" }
	| { phase: "open" }
	| { phase: "loading" }
	| { phase: "done"; result: { translation: string; notes?: string | null } }
	| { phase: "error" };

interface AiRefineBlockProps {
	refineState: AiRefineState;
	size?: "sm" | "md";
	onOpen: () => void;
	onSubmit: (hint: string) => void;
}

export const AiRefineBlock = ({
	refineState,
	size = "sm",
	onOpen,
	onSubmit,
	...
	if (refineState.phase === "idle") {
		return (
			<div
				className={`flex items-center justify-between gap-2 ${isSm ? "border-t-[0.5px] border-bd-1 px-3.5 py-2" : "border-t border-bd-1 px-4 py-2.5"}`}
			>
				<div
					className={`flex items-center gap-1 text-t-4 ${isSm ? "text-[8px]" : "text-[10.5px]"}`}
				>
					<span className="leading-[1.4]">{t("aiTranslation.disclaimer")}</span>
				</div>
				<Button
					size="bare"
					onClick={onOpen}
					className={`flex shrink-0 items-center gap-1 text-t-3 transition-colors hover:text-t-1 ${isSm ? "text-[11.5px]" : "text-[13px]"}`}
				>
					<MessageSquare
		...
		return (
			...
			>
				...
				<textarea
					...
					disabled={isLoading}
					rows={2}
					className={`w-full resize-none rounded-[6px] border-[0.5px] border-bd-2 bg-surf-2 px-2.5 py-1.5 text-t-1 placeholder:text-t-4 focus:border-acc focus:outline-none disabled:opacity-50 ${isSm ? "text-[12px]" : "text-[13px]"}`}
				/>
