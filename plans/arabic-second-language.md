...
2. **Backend AI-translation промпты хардкодят "Chechen":**
   `ai-translation.service.ts:480,485,507,529,546` — `"You are a Chechen language assistant..."`, поле `"notChechen"`, батч-промпт `"You are a Chechen-Russian language assistant"`. Существует только `targetLanguage` (язык **перевода**), нет `sourceLanguage` (язык **исходного слова** — сейчас всегда неявно чеченский).
...
> ⚠️ **Подтверждено ревью:** AI-cache — это не application-level ключ, а Prisma unique-constraint `@@unique([lemma, cacheType, targetLanguage])` (schema.prisma:1338). Без миграции арабское слово и его чеченский омограф коллизируют на `(lemma, WORD_ONLY, ru)` и будут перезаписывать чужой перевод в кэше. Это критический баг, а не опциональная доработка.
...
- **Выдача доступа** — через уже существующий UI overrides (`overrides-table.tsx` / `feature-flag-override-modal.tsx` → `POST /admin/feature-flags/overrides`). Админ выдаёт доступ себе и любому пользователю по email/id. **Специальный код-level "admin bypass" не делаем** — это лишняя сущность поверх уже готового механизма: админу, которому нужен доступ, достаточно выдать override самому себе тем же UI, которым он выдаёт его другим.
